...
{
	Sigmoid *s = malloc(sizeof(Sigmoid));
	s->x = NULL;
	s->out = NULL;
	return s;
}
...
Matrix *nn_sigmoid_forward(Sigmoid *s, Matrix *m)
{
	...
	for (size_t y = 0; y < m->rows; y++)
	{
		for (size_t x = 0; x < m->cols; x++)
		{
			result->data[y][x] = 1 / (1 + exp(-m->data[y][x]));
		}
	}
	s->x = m;
	s->out = result;
...
{
	Matrix *temp = ones(s->out->rows, s->out->cols);
	Matrix *sig_grad = element_wise_mats_product(s->out, subtract_mats(temp, s->out));
	free_mat(temp);
	Matrix *result = element_wise_mats_product(sig_grad, next_grad);
	free_mat(sig_grad);
	return result;
}
